...
export default function Sites() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sites'] }); setModal({ open: false, site: null }); },
  });
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <div className="flex gap-3">
            ...
            <GlassButton onClick={() => setModal({ open: true, site: null })}><Plus className="w-4 h-4" /> Add Site</GlassButton>
          </div>
        }
      />
            ...
            return (
              <div key={site.id} className="entity-card p-5 group">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <GlassButton variant="ghost" className="text-xs" onClick={() => setModal({ open: true, site })}><Pencil className="w-3 h-3" /> Edit</GlassButton>
                  <GlassButton variant="ghost" className="text-xs text-rose-400" onClick={() => { if (confirm('Delete this site?')) deleteMut.mutate(site.id); }}><Trash2 className="w-3 h-3" /> Delete</GlassButton>
                </div>
              </div>
            );
          })}
      ...
      <GlassModal open={modal.open} onClose={() => setModal({ open: false, site: null })} title={modal.site ? 'Edit Site' : 'New Site'}>
        <SiteForm site={modal.site} onSubmit={(d) => saveMut.mutate(d)} onCancel={() => setModal({ open: false, site: null })} />
      </GlassModal>
    </div>
  );
}